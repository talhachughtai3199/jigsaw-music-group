...
class UnAssignedFilesView: NSView {
    ...
    func initializeSettings(){ 
        ...
        config.unAssignedFilesView = self
        loadFromNib()
        updateCount()
        updateUI()
    ...
    func refreshData(){
        ...
        for item in ScanDirectoryFiles.shared.files.filter({$0.itemType == .Unassigned}){
            items.append(item)
        }
    ...
    private func loadFromNib() {
        ...
        Bundle.main.loadNibNamed("UnAssignedFilesView", owner: self, topLevelObjects: &topLevelObjects)
...
extension UnAssignedFilesView: NSCollectionViewDelegate, NSCollectionViewDataSource {
   
   func collectionView(_ collectionView: NSCollectionView, numberOfItemsInSection section: Int) -> Int {
       var itemsCount = 0
       let collectionViewName = collectionView.accessibilityIdentifier()
       if(collectionViewName == "UnAssignedFilesCollectionIdentifier"){
   ...
   func collectionView(_ collectionView: NSCollectionView, itemForRepresentedObjectAt indexPath: IndexPath) -> NSCollectionViewItem {
       let item = collectionView.makeItem(withIdentifier: NSUserInterfaceItemIdentifier("UnAssignedFilesCollectionViewItem"), for: indexPath)
       guard let safeItem = item as? UnAssignedFilesCollectionViewItem else {return item}
   ...
   func collectionView(_ collectionView: NSCollectionView, didSelectItemsAt indexPaths: Set<IndexPath>) {
       ...
       if(collectionViewName == "UnAssignedFilesCollectionIdentifier"){
           if let itemIndex = indexPaths.first{
               guard let selectedItem = collectionView.item(at: itemIndex) as? UnAssignedFilesCollectionViewItem else {
                   return
               }
